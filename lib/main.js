...
function openNeuron(someText) {
	tabs.open({
		url: "http://www.livarava.com/neuron/new/?liva="+encodeURIComponent(someText[0])+"&rava="+encodeURIComponent(someText[1])
...
function checkAuthorization(neuron, axon){
	var pageWorker = require("sdk/page-worker").Page({
		...
		contentURL: "http://www.livarava.com/"
	});
	
	pageWorker.port.on("first", function(authoriz){
		console.log("before refer");
		if (authoriz=="yes"){
			console.log("authorization: "+authoriz);
			add_form.hide();
			openNeuron([neuron, axon]);
		}
		else{
			console.log("authorization: "+authoriz);
			add_form.hide();
